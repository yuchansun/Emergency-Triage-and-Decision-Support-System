...
function App() {
  ...
  const openVoiceConsentPopup = (): Promise<"agree" | "disagree" | "blocked"> => {
...
<body>
  ...
  const handleDisagreeVoiceConsent = () => {
    setVoiceConsented(false);
    localStorage.setItem("voiceConsented", "false");
    setShowVoiceConsentModal(false);
    setStage("main");
      ...
      <main className="flex-1 relative overflow-y-auto bg-[#F8FAFC]">
        ...
        {stage === "triageReport" && (
          ...
          <VoiceConsentModal
            ...
            onAgree={handleAgreeVoiceConsent}
            onDisagree={handleDisagreeVoiceConsent}
          />
      </main>
    </div>
  );
